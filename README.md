<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=black940514&color=4C8EDA&style=flat-square&label=Views)

# kimtaeyoun

**AI Engineer** · Computer Vision · ITS

</div>

---

## About

ITS(지능형교통시스템) 분야에서 영상 분석 업무를 하고 있습니다.

회사에서는 차량 Re-ID, 영상 보정, 도로 토폴로지 분석 등을 담당하고 있고, 개인적으로는 강화학습이랑 논문 읽는 걸 좋아합니다.

**ITS World Congress 2026**에 차량 재식별 시스템 관련 논문을 제출했습니다.

올해 대학원에 입학했습니다. 회사 다니면서 공부하는 게 쉽진 않지만, 하고 싶은 연구를 제대로 해보고 싶어서요.

---

## Tech

**Vision**: PyTorch, OpenCV, YOLO, TransReID, SAM, FAISS

**ML**: LightGBM, XGBoost, CatBoost, SHAP, WTTE-RNN

**LLM**: LangChain, RAG, Claude, OpenAI

**Infra**: Docker, FastAPI, uv

---

## Projects

### 업무 관련

| Project | Description | Highlight |
|---------|-------------|-----------|
| [ai-vehicle-reid](https://github.com/black940514/ai-vehicle-reid) | ITS 차량 재식별 시스템 (TransReID, FAISS) | **73.62% 복구율**, ITS World Congress 논문 |
| [its-maintenance](https://github.com/black940514/its-maintenance) | ITS 장비 고장 예측 및 RUL 추정 | **F1 0.85**, MAE 15일 |
| [road_topology](https://github.com/black940514/road_topology) | 도로 토폴로지 분할 (YOLOv8 + SAM) | BoxMOT, TSP6K |

### 개인 프로젝트

| Project | Description |
|---------|-------------|
| [MOT-Algorithm-Study](https://github.com/black940514/MOT-Algorithm-Study) | 6개 MOT 알고리즘 비교 연구 (22개 실험, 5000+ 프레임) |
| [ai-code-mentor](https://github.com/black940514/ai-code-mentor) | Claude Code Skill - AI 코드 이해 교육 도구 |
| [game_rl](https://github.com/black940514/game_rl) | PPO 강화학습 게임 AI |
| [tistory_writer](https://github.com/black940514/tistory_writer) | AI 논문 리뷰 자동 포스팅 |
| [upbit_black](https://github.com/black940514/upbit_black) | 암호화폐 자동매매 시스템 |
| [chatbot_project](https://github.com/black940514/chatbot_project) | 스마트스토어 RAG 챗봇 |

---

## Stats

<div align="center">

<img src="https://github-readme-stats-sigma-five.vercel.app/api?username=black940514&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117" width="49%" />
<img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=black940514&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117" width="49%" />

</div>

---

## Contact

- Blog: [ty-analyst.tistory.com](https://ty-analyst.tistory.com/) (107+ posts)
- Email: xodus4352@naver.com
